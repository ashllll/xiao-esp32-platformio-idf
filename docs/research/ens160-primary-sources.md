...
DFRobot 中文 Wiki 对 SEN0515 模块给出的规格如下。这里的 `3.3 V` 和 `29 mA` 是模块使用入口的资料；`29 mA` 与芯片 Standard 模式典型电流一致。电源设计应留出瞬态裕量，并在实物上测量供电压降。
...
| Standard 最大电流 | — | §4 Table 3 未单独给出最大值；DFRobot SEN0515 Wiki 同样标注 29 mA |
